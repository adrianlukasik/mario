    ...
    def draw(self, screen, game_position):
        ...
        for i in range(len(self.blocks)):
            for j in range(len(self.blocks[i])):
                ...
                position = Position(
                    (j * ELEMENT_SIZE - game_position.get_x(), i * ELEMENT_SIZE)
                )
                if block == "b":
                    ...
                    if self.blocks[i + 1][j] == "#":
                        ...
                        box = pygame.Rect(
                            position.get_x() + ELEMENT_SIZE // 2 - 3,
                            position.get_y(),
                            6,
                            ELEMENT_SIZE,
                        )
                    ...
                    if self.blocks[i - 1][j] == "*":
                        screen.blit(
                            Board.BALL, (position.get_x() + 8, position.get_y())
                        )
                    elif self.blocks[i - 2][j] == "*":
                        screen.blit(
                            Board.FLAG,
                            (
                                position.get_x() - ELEMENT_SIZE // 2,
        ...
        castle_position.change_position(
            Board.VECTOR_CASTLE_POSITION[0], Board.VECTOR_CASTLE_POSITION[1]
        )
